...
// --- Construction Contracts Routes (Professional SCA Standards) ---
...
app.post('/api/construction-contracts', authenticate, async (req, res) => {
    ...
    try {
        ...
        await logActivity(req.user.id, 'CREATE', 'CONSTRUCTION_CONTRACT', result.id, `إنشاء عقد مقاولات احترافي #${result.contractNumber}`);
        res.json(result);
    } catch (error) {
...
app.put('/api/construction-contracts/:id', authenticate, async (req, res) => {
    ...
    try {
        if (body.items && Array.isArray(body.items)) {
            ...
            await logActivity(req.user.id, 'UPDATE_STATUS', 'CONSTRUCTION_CONTRACT', contract.id, `تحديث حالة عقد مقاولات #${contract.contractNumber}`);
            res.json(contract);
        }
    } catch (error) {
        res.status(500).json({ error: error.message });
